...
#define FLAGS(T) T;\
    ...
    static inline T&   operator    &=    (T& x, T y)      {    return x = x & y;    }; \
    static inline T&   operator    |=    (T& x, T y)      {    return x = x | y;    }; \
    static inline T&   operator    ^=    (T& x, T y)      {    return x = x ^ y;    }; \
    ...
    enum class T
			...
			constexpr int RECEIVE_REQ_MAX_BYTES  = 255;
			constexpr int RECEIVE_FULL_MAX_BYTES = 6850;
			
			
			constexpr int SEND_REQ_MAX_BYTES  = 255;
			constexpr int SEND_FULL_MAX_BYTES = 6850;
